...
const GUILDS = {
    ...
    6: { number: 6, name: 'Taberna', nameES: 'Taberna' },
    8: { number: 8, name: 'Granja', nameES: 'Granja' },
...
const EVENT_DEFINITIONS = {
    ...
    GUILD_FOUNDATIONS: [
        ...
        { type: EVENT_TYPES.GUILD_FOUNDATION, guild: 6, name: 'Fundación: Taberna' },
        { type: EVENT_TYPES.GUILD_FOUNDATION, guild: 8, name: 'Fundación: Granja' },
...
// Treasure deck distribution (12 cards total)
function createTreasureDeck() {
    const deck = [];
    // 3 Wealth cards with 3 coins
    for (let i = 0; i < 3; i++) {
        deck.push({ type: TREASURE_TYPES.WEALTH, vp: 0, coinValue: 3 });
    }
    // 3 Wealth cards with 4 coins
    for (let i = 0; i < 3; i++) {
        deck.push({ type: TREASURE_TYPES.WEALTH, vp: 0, coinValue: 4 });
    }
    // 3 Common treasures (1 VP)
    for (let i = 0; i < 3; i++) {
        deck.push({ type: TREASURE_TYPES.COMMON, vp: 1 });
    }
    // 3 Rare treasures (2 VP)
    for (let i = 0; i < 3; i++) {
